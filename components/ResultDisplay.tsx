        ...
        <section className="mt-12 bg-slate-800/60 backdrop-blur-sm rounded-xl shadow-2xl p-6 md:p-8 border border-slate-600 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="text-center">
                    <h3 className="text-xl font-semibold mb-4 text-slate-100">{originalLabel}</h3>
                    <img src={originalImage} alt="Original" className="rounded-lg shadow-lg mx-auto max-h-96" />
                </div>
                <div className="text-center">
                    <h3 className="text-xl font-semibold mb-4 text-slate-100">{editedLabel}</h3>
                    <img src={editedImage} alt={`Edited with prompt: ${prompt}`} className="rounded-lg shadow-lg mx-auto max-h-96" />
                    <p className="text-slate-200 mt-3 italic text-sm">{promptLabel}: "{prompt}"</p>
                </div>
            </div>
        </section>
    );
